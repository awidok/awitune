...
### GPU Memory Constraints for Tree Models (CatBoost, XGBoost, LightGBM)
- **CatBoost on GPU consumes ALL available GPU memory by default**. This causes OOM errors on shared infrastructure.
- **MANDATORY**: For any experiment using CatBoost on GPU, explicitly limit GPU memory:
  ```python
  ...
  model = CatBoostClassifier(
      task_type="GPU",
      devices="0",
      gpu_ram_limit=36*1024,  # Limit to 36GB (in MB)
      # ... other params
  )
  ```
- Alternative: Use `task_type="CPU"` for CatBoost — it's still fast and avoids GPU memory issues.
- Same applies to XGBoost (`tree_method="hist"` with `gpu_id`) and LightGBM (`device="gpu"`).
